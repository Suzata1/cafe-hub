...
const NavBar = () => {
  ...
  return (
    <>
      {/* Navbar */}
      <div className='bg-red-600 sticky top-0 px-4 text-white w-full z-50'>
        <div className='max-w-[1400px] mx-auto'>
          <div className='flex justify-between items-center h-16 w-full'>
            ...
            <div className='flex items-center gap-2'>
              ...
              <Link href='/' className='font-extrabold text-white text-2xl'>BurgerHub</Link>
            </div>
      ...
      <div className={`fixed top-0 left-0 h-full w-full bg-white text-black z-40 transform transition-transform duration-300 ${sidebarOpen ? 'translate-x-0' : '-translate-x-full'}`}>
        ...
        <nav className="flex flex-col p-4 space-y-4">
          {socialMedias.map((social) => (
            <Link key={social.id} href={social.href} onClick={() => setSidebarOpen(false)}>
