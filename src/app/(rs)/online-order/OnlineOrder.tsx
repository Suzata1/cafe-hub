...
// export default function OnlineOrder() {
//   const [paymentMethod, setPaymentMethod] = useState('');
//   const [paymentResponse] = useState<PaymentResponse | null>(null); 
...
export default function OnlineOrder() {
  ...
  const handleSubmit = () => {
    ...
    const items = formData.orderDetails
      .split(',')
      .map((item) => item.trim())
      .filter(Boolean);
  ...
  const handleCheckout = async () => {
    if (!orderSummary) {
      alert('Please submit order first');
      return;
    }
    ...
    // 🔐 CALL YOUR OWN API ROUTE HERE
    try {
      const res = await fetch('/api/khalti', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ amount: orderSummary.total })
      });
      ...
      <div className="grid md:grid-cols-3 gap-12">
        ...
        <div className="space-y-6">
          ...
          <div className="border p-4 shadow rounded-md">
            ...
            {orderSummary ? (
              <>
                ...
                <p>
                  <span className="font-medium">Food names:</span>{' '}
                  {orderSummary.foodNames}
                </p>
                <p className="mt-2">
          ...
          <div className="bg-gray-100 p-4 shadow rounded-md">
            ...
            <div className="grid grid-cols-2 gap-2">
              {['Khalti', 'Credit Card', 'GPay', 'Esewa'].map(
                (method) => (
                  <label key={method} className="flex items-center gap-2">
                    <input
                      type="radio"
